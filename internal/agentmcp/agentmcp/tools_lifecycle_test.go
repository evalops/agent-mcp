...
func TestToolRegisterRejectsWhenSessionLimitReached(t *testing.T) {
	var identityCalls atomic.Int32
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		identityCalls.Add(1)
		w.WriteHeader(http.StatusCreated)
		_ = json.NewEncoder(w).Encode(clients.AgentSession{
			AgentID: "agent_over_limit", AgentToken: "tok_new", ExpiresAt: time.Now().Add(time.Hour),
		})
	}))
	defer srv.Close()

	deps := newTestDeps(srv)
	deps.Config.Session.MaxActive = 1
	deps.Sessions.Set("existing", &SessionState{AgentID: "agent_existing"})
	rc := newRequestContext(deps, "new_session")

	_, _, err := rc.toolRegister(context.Background(), nil, registerInput{
		AgentType: "claude-code",
		Surface:   "cli",
	})
	if err == nil || !strings.Contains(err.Error(), "active session limit reached") {
		t.Fatalf("expected active session limit error, got %v", err)
	}
	if identityCalls.Load() != 0 {
		t.Fatalf("identity should not be called after preflight limit rejection, got %d calls", identityCalls.Load())
	}
}
...
func TestToolRegisterWithRegistry(t *testing.T) {
	...
	// Mock Agent Registry ConnectRPC server.
	...
	if mockRegistry.registerCalled != 1 {
		t.Fatalf("expected 1 agent-registry register call, got %d", mockRegistry.registerCalled)
	}
}
...
func TestToolHeartbeatWithRegistry(t *testing.T) {
	...
	// Agent Registry heartbeat is now fire-and-forget; poll until the background goroutine completes.
	deadline := time.After(2 * time.Second)
	for mockRegistry.heartbeatCalled.Load() < 1 {
		select {
		case <-deadline:
			t.Fatalf("timed out waiting for background agent-registry heartbeat; called=%d", mockRegistry.heartbeatCalled.Load())
		default:
...
func TestToolHeartbeatBackgroundRegistryCallUsesConfiguredTimeout(t *testing.T) {
	...
	mockRegistry := &mockAgentService{
		heartbeatFunc: func(ctx context.Context, _ *connect.Request[agentsv1.HeartbeatRequest]) (*connect.Response[agentsv1.HeartbeatResponse], error) {
			...
			case <-time.After(200 * time.Millisecond):
				done <- timeoutResult{err: errors.New("background agent-registry heartbeat did not time out"), elapsed: time.Since(start), hasDeadline: hasDeadline}
	...
	case result := <-done:
		if result.err == nil {
			t.Fatal("expected background agent-registry heartbeat to be canceled by timeout")
		}
		if result.elapsed >= 150*time.Millisecond {
			t.Fatalf("expected timeout-bound background agent-registry heartbeat, got elapsed=%v err=%v", result.elapsed, result.err)
		}
		if !result.hasDeadline {
			t.Fatal("expected background agent-registry heartbeat context to carry a deadline")
		}
	case <-time.After(500 * time.Millisecond):
		t.Fatal("timed out waiting for background agent-registry heartbeat timeout")
	}
}
