...
func serverForRequest(deps *Deps, r *http.Request) *mcpsdk.Server {
	...
	mcpsdk.AddTool(server, &mcpsdk.Tool{
		...
		Description: "Register this agent with EvalOps \u2014 creates identity session and agent-registry presence in one call. Call this at the start of every session before using any other EvalOps tools",
	}, rc.toolRegister)

	mcpsdk.AddTool(server, &mcpsdk.Tool{
		Name:        "evalops_heartbeat",
		Description: "Heartbeat the agent session \u2014 rotates identity token and updates agent-registry presence. Call this every 60 seconds to maintain session liveness",
	}, rc.toolHeartbeat)

	mcpsdk.AddTool(server, &mcpsdk.Tool{
		Name:        "evalops_deregister",
		Description: "Deregister the agent \u2014 revokes identity session and removes agent-registry presence",
	}, rc.toolDeregister)
	...
	mcpsdk.AddTool(server, &mcpsdk.Tool{
		Name:        "evalops_list_tools",
		Description: "List EvalOps and integration tool capabilities available through agent-mcp, including namespace, risk, approval, cost, and provenance metadata",
	}, rc.toolListTools)
