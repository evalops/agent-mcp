...
type SessionBackend interface {
	...
	SetIfUnderLimit(sessionID string, state *SessionState, maxActive int) bool
...
func (s *MemorySessionStore) Set(sessionID string, state *SessionState) {
	_ = s.SetIfUnderLimit(sessionID, state, 0)
}

func (s *MemorySessionStore) SetIfUnderLimit(sessionID string, state *SessionState, maxActive int) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, exists := s.store[sessionID]; !exists && maxActive > 0 && len(s.store) >= maxActive {
		return false
	}
	s.store[sessionID] = cloneSessionState(state)
	return true
}
...
func (s *MemorySessionStore) All() map[string]*SessionState {
	...
	for k, v := range s.store {
		snapshot[k] = cloneSessionState(v)
	}
	return snapshot
}
