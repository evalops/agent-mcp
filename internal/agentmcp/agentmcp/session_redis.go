...
func (s *RedisSessionStore) SetIfUnderLimit(sessionID string, state *SessionState, maxActive int) bool {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	...
	if existed == 0 && maxActive > 0 && s.ActiveCount() >= maxActive {
		return false
	}
	if err := s.client.Set(ctx, sessionKeyPrefix+sessionID, data, ttl).Err(); err != nil {
		return false
	}
