...
func (rc *requestContext) toolReportUsage(
	...
	clonedMsg := &meterv1.RecordUsageRequest{
		AgentId:      agentID,
		Surface:      surface,
		EventType:    eventType,
		Model:        input.Model,
	...
	if !rc.deps.RunBackground("meter.record_usage", func() {
		...
		rc.logger.Warn("meter usage skipped: background task capacity exhausted", "agent_id", agentID, "request_id", requestID)
		return nil, reportUsageOutput{Recorded: false, Async: false}, nil
	}
