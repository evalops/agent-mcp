...
func TestDecisionString(t *testing.T) {
	tests := []struct {
		...
		{governancev1.ActionDecision_ACTION_DECISION_UNSPECIFIED, "deny"},
	}
...
func TestCheckActionRequiresActiveSessionWhenGovernanceConfigured(t *testing.T) {
	mockGov := &mockGovernanceService{
	...
	result, out, err := rc.toolCheckAction(context.Background(), nil, checkActionInput{ActionType: "Bash"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if out.Decision != "deny" {
		t.Fatalf("expected deny without active session, got %#v", out)
	}
}
...
func TestCheckApprovalEmptyStateDefaultsPending(t *testing.T) {
	mockApprovals := &mockApprovalService{approvalStates: map[string]string{"apr_empty": ""}}
	_, handler := approvalsv1connect.NewApprovalServiceHandler(mockApprovals)
	appSrv := httptest.NewServer(handler)
	defer appSrv.Close()
	...
	_, out, err := rc.toolCheckApproval(context.Background(), nil, checkApprovalInput{ApprovalID: "apr_empty"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if out.State != "pending" {
		t.Fatalf("expected pending for empty approval state, got %s", out.State)
	}
}
